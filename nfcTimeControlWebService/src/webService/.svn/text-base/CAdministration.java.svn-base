...
import webService.request.CRequestAdministration;
import webService.response.CResponse;

import db.dao.AdException;
import db.dao.CCompanyDAOImpl;
...
import db.entity.CConstrains.locationStatus;
...
public class CAdministration {
	...
	/**
	 * g��wna metoda rozpoznaj�ca i przetwarzaj�ca rejestracje nowego TAGU
	 * @param request
	 * @return status
	 * @throws AdException
	 */
	...
	public CResponse registerNewTag(CRequestAdministration request) throws AdException{
		...
		String locationName = request.getLocation_name();
	    ...
	    System.out.println("Cmp name " + request.getCompany_name());	
		...
		try {
			...
			if(user != null && CUtility.checkUser(user, pass, imei, imsi )) {
				...
				else return new CResponse(responseStatus.WRONG_PASS.getText(), null,null); // zwracam gdy nie ma wystarczaj�cych uprawnie�
			}
			else {
